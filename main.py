...
class UI:
    def __init__(self, master):
        ...
        self.get_outputfile = Entry(self.middleframe, justify=LEFT, textvariable=outputfile)
        self.botframe = Frame(master, bg="#2a2a2a")
        self.botframe.pack(expand=TRUE, side=BOTTOM, fill=BOTH)
        self.select_enc = Radiobutton(self.botframe, text="Encrypt", var=command, value=TRUE)
        self.select_dec = Radiobutton(self.botframe, text="Decrypt", var=command, value=FALSE)
        ...
        self.get_outputfile.grid(row=1, column=2, padx=5, pady=5, sticky=W+E)
